...
static void clear_screen() { printf("\033[H\033[J"); printf(RESET); } // deletes screen and sets cursor to top line
...
static void print_selected(char *text){  printf(NEW_WHITE "  > %s < " RESET, text);  }
...
static char *calculate_padding(const char *longest_string, const char *string_to_pad){
    ...
    int padding = (longest_string_len - string_to_pad_len) / 2;
